...
const shishenMap: Record<string, string[]> = {
  '甲': ['比肩', '劫财', '食神', '伤官', '偏财', '正财', '偏官', '正官', '偏印', '正印'],
  '乙': ['劫财', '比肩', '伤官', '食神', '正财', '偏财', '正官', '偏官', '正印', '偏印'],
  '丙': ['比肩', '劫财', '偏印', '正印', '食神', '伤官', '偏财', '正财', '偏官', '正官'],
  '丁': ['劫财', '比肩', '正印', '偏印', '伤官', '食神', '正财', '偏财', '正官', '偏官'],
  '戊': ['比肩', '劫财', '食神', '伤官', '偏财', '正财', '偏官', '正官', '偏印', '正印'],
  '己': ['劫财', '比肩', '伤官', '食神', '正财', '偏财', '正官', '偏官', '正印', '偏印'],
  '庚': ['比肩', '劫财', '偏印', '正印', '食神', '伤官', '偏财', '正财', '偏官', '正官'],
  '辛': ['劫财', '比肩', '正印', '偏印', '伤官', '食神', '正财', '偏财', '正官', '偏官'],
  '壬': ['比肩', '劫财', '食神', '伤官', '偏财', '正财', '偏官', '正官', '偏印', '正印'],
  '癸': ['劫财', '比肩', '伤官', '食神', '正财', '偏财', '正官', '偏官', '正印', '偏印']
};

// 计算十神
function getShishen(gan: string, dayGan: string): string {
  const ganIdx = ganOrder[gan];
  const dayIdx = ganOrder[dayGan];
  if (ganIdx === undefined || dayIdx === undefined) return '';

  // 十神关系：看天干与日干的关系
  // 同我者比劫（阴阴不同），我生者食伤，我克者财，克我者官杀，生我者印
  const diff = (ganIdx - dayIdx + 10) % 10;

  // 十神顺序：比肩0 劫财1 食神2 伤官3 偏财4 正财5 偏官6 正官7 偏印8 正印9
  const shishenList = shishenMap[dayGan] || shishenMap['甲'];
  return shishenList[diff];
}

// 支藏天干表
const zhichangMap: Record<string, string[]> = {
  '子': ['癸'],
  '丑': ['己', '癸', '辛'],
  '寅': ['甲', '丙', '戊'],
  '卯': ['乙'],
